...
def parse_cmd_options(argv):
    ...
    # parser.add_argument('--search_space', type=str, default=None, help='.py file to specify the search space.')
...
def get_new_random_structure_str(structure_str, num_replaces=1, channel=64):

    def channel_mutator(channel):
        channel = round(channel + 8 * random.choice([-2, -1, 0, 1, 2]))
        if channel < 8 :
            channel = 8
    ...
    for replace_count in range(num_replaces):
        ...
        def sublayer_mutator(sublayer, block_list_len):
            ...
            if sublayer < 1:
                sublayer_1 = 1
                sublayer_2 = 0
            elif sublayer > 2 and block_list_len == 7:
                sublayer_1 = 2
                sublayer_2 = 0
            elif sublayer > 2:
        ...
        def layer_type_mutator():
            # random_type = random.randint(5, 7)
            # only SuperResK3K3 can get the best result
            random_type = 5
        ...
        if len(params_block) == 4 :
            ...
            block_type              = layer_type_mutator()
            params_block[1]         = channel_mutator(params_block[1])
            params_block[3]         = channel_mutator(params_block[3])
            sublayer_1, sublayer_2  = sublayer_mutator(params_block[4], len(block_list))
    ...
    # params_block[1]     = str(new_channel)
    # params_block[1]     = str(channel_mutator(int(params_block[1])))
    new_student_str_1   = block_type + "(" + ','.join(params_block) + ")"
    block_list[0]       = new_student_str_1
    ...
    # print(new_random_structure_str)
...
def compute_nas_score(AnyPlainNet, random_structure_str, gpu, args):
    ...
    if not args.zero_shot_score == 'TPC_hardware':
        no_create = False
    ...
    try:
        if args.zero_shot_score == 'Zen':
            ...
            the_nas_core_info = compute_tpc_score.compute_nas_score(model=the_model, gpu=gpu,
                                                                    ...
                                                                    repeat=1)
            the_nas_core = the_nas_core_info['avg_nas_score']
        elif args.zero_shot_score == 'TPC_hardware':
...
def main(args, argv):
    ...
    for loop_count in range(args.evolution_max_iter):
        ...
        if len(popu_structure_list) <= 10:
            random_structure_str = get_new_random_structure_str(structure_str=initial_structure_str, num_replaces=1, channel=args.channel)
        else:
            tmp_idx = random.randint(0, len(popu_structure_list) - 1)
            tmp_random_structure_str = popu_structure_list[tmp_idx]
            random_structure_str = get_new_random_structure_str(structure_str=tmp_random_structure_str, num_replaces=2, channel=args.channel)
