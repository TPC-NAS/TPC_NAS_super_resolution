...
class SubPixelConvolutionalBlock(PlainNetBasicBlockClass):
    def __init__(self, in_channels=None, out_channels=None, kernel_size=3, bias=True, scaling_factor=2, copy_from=None, no_create=False,  **kwargs):

        super(SubPixelConvolutionalBlock, self).__init__(**kwargs)
        self.no_create = no_create
        ...
        self.in_channels    = in_channels
        self.out_channels   = out_channels * (scaling_factor ** 2)
        self.groups         = 1
        # A convolutional layer that increases the number of channels by scaling factor^2, followed by pixel shuffle and PReLU
        self.conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels * (scaling_factor ** 2), kernel_size=kernel_size, padding=kernel_size // 2, bias=True)
    ...
    def __str__(self):
        return 'SubPixelConvolutionalBlock({},{},{})'.format(self.in_channels, self.kernel_size, int(self.scaling_factor))

    def __repr__(self):
        return 'SubPixelConvolutionalBlock({}|{},{},{})'.format(self.block_name, self.in_channels, self.kernel_size, int(self.scaling_factor))
...
class MeanShift(PlainNetBasicBlockClass):
    def __init__(self, mean_rgb=None, sub=None, copy_from=None, no_create=False, **kwargs):
    ...
    def __repr__(self):
        return 'MeanShift({})'.format(self.block_name)
    ...
    def __init__(self, out_channels=None, copy_from=None, no_create=False, **kwargs):
        ...
        if copy_from is not None:
            ...
            if no_create:
                ...
                self.netblock = nn.BatchNorm2d(num_features=self.out_channels)
                # self.netblock = nn.InstanceNorm2d(num_features=self.out_channels)
        ...
        if copy_from is not None:
            ...
            if no_create or self.in_channels == 0 or self.out_channels == 0 or self.kernel_size == 0 or self.stride == 0:
                ...
                self.netblock = nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels,
                                          ...
                                          padding=self.padding, bias=True, groups=self.groups)
...
class RELU_inplace_true(PlainNetBasicBlockClass):
    def __init__(self, out_channels, no_create=False, **kwargs):
        super(RELU_inplace_true, self).__init__(**kwargs)
        self.in_channels = out_channels
        self.out_channels = out_channels
        self.no_create = no_create
    ...
    @classmethod
    def create_from_str(cls, s, no_create=False, **kwargs):
        assert RELU_inplace_true.is_instance_from_str(s)
        idx = _get_right_parentheses_index_(s)
        assert idx is not None
        param_str = s[len('RELU_inplace_true('):idx]
        # find block_name
        tmp_idx = param_str.find('|')
        if tmp_idx < 0:
            tmp_block_name = 'uuid{}'.format(uuid.uuid4().hex)
        ...
        out_channels = int(param_str)
        return RELU_inplace_true(out_channels=out_channels, no_create=no_create, block_name=tmp_block_name), s[idx+1:]
...
class ResBlock(PlainNetBasicBlockClass):
    ...
    def get_FLOPs(self, input_resolution):
        ...
        # if self.in_channels != self.out_channels:
        #     the_flops += self.in_channels * self.out_channels * the_res ** 2
...
class ResBlockProj(PlainNetBasicBlockClass):
    ...
    def __init__(self, block_list, in_channels=None, stride=None, no_create=False, **kwargs):
        ...
        conv_counter = 1 # input data
        for inner_block in self.block_list:
            if isinstance(inner_block, ResBlock):
                conv_counter += 1

        self.in_channel_counter = (conv_counter)

        self.conv = nn.ModuleList()
        for i in range(conv_counter):
            if i == conv_counter - 1 :
                self.conv.append(nn.Conv2d(self.out_channels, self.out_channels//2, 3, 1, 1, bias=True))
            elif i == 0:
                self.conv.append(nn.Conv2d(self.in_channels, self.out_channels//2, 1, 1, bias=True))
            else:
                self.conv.append(nn.Conv2d(self.out_channels, self.out_channels//2, 1, 1, bias=True))

        # print(conv_counter)
        self.concat_conv = nn.Conv2d((self.out_channels//2)*(conv_counter), self.out_channels, 1, 1, bias=True)

        self.act = nn.LeakyReLU(0.05, True)
        self.act_no_inplace = nn.LeakyReLU(0.05, False)
        self.ESA = ESA(out_channels=self.out_channels)
    ...
    def forward(self, x):
        ...
        output_1 = self.conv[conv_counter](output)
        output_1 = self.act(output_1)
        output_buffer.append(output_1)
        conv_counter += 1
    ...
    def get_FLOPs(self, input_resolution):
        ...
        the_flops += self.in_channel_counter * self.in_channels * (self.in_channels//2) * the_res ** 2
        the_flops += (self.in_channel_counter) * (self.in_channels//2) * self.in_channels * the_res ** 2
        the_flops += self.ESA.get_FLOPs(the_res)
    ...
    def get_model_size(self):
        ...
        the_size += self.in_channel_counter * self.in_channels * (self.in_channels//2)
        the_size += (self.in_channel_counter) * (self.in_channels//2) * self.in_channels
        the_size += self.ESA.get_model_size()
...
class ESA(PlainNetBasicBlockClass):
    def __init__(self, out_channels=None, copy_from=None,
                 no_create=False, **kwargs):
        super(ESA, self).__init__(**kwargs)
        self.no_create = no_create
        ...
        if copy_from is not None:
            ...
            n_feats = self.in_channels
            f = self.in_channels // 4
    ...
    def get_FLOPs(self, input_resolution):
        ...
        the_flop = 0
        the_flop += n_feats * f * input_resolution ** 2
        the_flop += f * f * 9 * input_resolution ** 2 // 4
        new_resolution = (input_resolution - 2) // 2
        new_resolution = (input_resolution - 6) // 3
        the_flop += f * f * 9 * new_resolution ** 2
        the_flop += f * f * 9 * new_resolution ** 2
        the_flop += f * f * 1 * input_resolution
        the_flop += n_feats * f * input_resolution ** 2

        # the_flop += f * f * input_resolution **2
        # the_flop +=
        return the_flop
...
def register_netblocks_dict(netblocks_dict: dict):
    this_py_file_netblocks_dict = {
        ...
        'RELU': RELU,
        'RELU_inplace_true': RELU_inplace_true,
        'ResBlock': ResBlock,
